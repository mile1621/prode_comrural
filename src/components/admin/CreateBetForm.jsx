    ...
    <div className="flex flex-col gap-1.5">
      ...
      <input
        ...
        onFocus={e => {
          ...
          e.target.style.boxShadow   = '0 0 0 3px rgba(235,195,43,.12)'
        }}
        onBlur={e => {
...
function FilterChip({ active, onClick, children }) {
  return (
    <button
      ...
      onMouseEnter={e => { if (!active) { e.currentTarget.style.borderColor = '#0c182b'; e.currentTarget.style.color = '#0c182b' } }}
      onMouseLeave={e => { if (!active) { e.currentTarget.style.borderColor = '#e8dfd0'; e.currentTarget.style.color = '#5f6e8a' } }}
    >
      {children}
    </button>
  )
      ...
      <div className="flex flex-col gap-2">
        ...
        {fasesDisponibles.length > 1 && (
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="font-body font-semibold uppercase w-14"
              style={{ fontSize: 10, color: '#a8b2c4', letterSpacing: '.1em' }}>Fase</span>
        ...
        <div className="flex items-center justify-between gap-2 text-xs font-body">
          <button type="button" onClick={toggleVisibles} disabled={partidosFiltrados.length === 0}
            className="transition-colors" style={{ color: '#c99f16' }}
